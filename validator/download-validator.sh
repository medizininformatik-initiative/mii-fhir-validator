...
curl -L -o validator_cli.jar "$DOWNLOAD_URL"

if [ -f "validator_cli.jar" ]; then
    echo "✓ Download complete!"
    echo "✓ File: validator_cli.jar"
    ls -lh validator_cli.jar
    
    # Test the JAR by checking help output
    echo ""
    echo "Testing validator..."
    if java -jar validator_cli.jar -help 2>&1 | grep -q "FHIR Validation"; then
